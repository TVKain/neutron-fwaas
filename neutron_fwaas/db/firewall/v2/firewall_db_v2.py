...
class FirewallDefaultParameterExists(exceptions.InUse):
    ...
    message = (
        "Operation cannot be performed since '%(name)s' "
...
class FirewallDefaultObjectUpdateRestricted(FirewallDefaultParameterExists):
    message = (
        "Operation cannot be performed on default object "
...
class HasDescription(object):
    description = sa.Column(sa.String(db_constants.LONG_DESCRIPTION_FIELD_SIZE))


class FirewallRuleV2(
    ...
    action = sa.Column(sa.Enum("allow", "deny", "reject", name="firewallrules_action"))
...
class FirewallGroup(
    ...
    port_associations = orm.relationship(
        ...
        backref=orm.backref(
            "firewall_group_port_associations_v2", cascade="all, delete"
        ),
    )
    name = sa.Column(sa.String(db_constants.NAME_FIELD_SIZE))
...
def _list_firewall_policies_result_filter_hook(query, filters):
    ...
    if values:
        ...
        query = query.filter(FirewallPolicyRuleAssociation.firewall_rule_id.in_(values))
...
class FirewallPluginDb(object):
    ...
    def _validate_fwr_src_dst_ip_version(self, fwr, fwr_db=None):
        ...
        if fwr.get("destination_ip_address", None):
            dst_version = netaddr.IPNetwork(fwr["destination_ip_address"]).version
    ...
    def _make_firewall_rule_dict(self, firewall_rule, fields=None, policies=None):
        ...
        res = {
            ...
            "destination_ip_address": firewall_rule["destination_ip_address"],
        ...
        if hasattr(firewall_rule.standard_attr, "id"):
            ...
            resource_extend.apply_funcs("firewall_rules", res, firewall_rule)
    ...
    def _make_firewall_policy_dict(self, firewall_policy, fields=None):
        ...
        if hasattr(firewall_policy.standard_attr, "id"):
            ...
            resource_extend.apply_funcs("firewall_policies", res, firewall_policy)
        return db_utils.resource_fields(res, fields)

    def _make_firewall_group_dict(self, firewall_group_db, fields=None):
        fwg_ports = [
            port_assoc.port_id for port_assoc in firewall_group_db.port_associations
        ]
        res = {
            "id": firewall_group_db["id"],
            "tenant_id": firewall_group_db["tenant_id"],
            "name": firewall_group_db["name"],
            "description": firewall_group_db["description"],
            "ingress_firewall_policy_id": firewall_group_db[
                "ingress_firewall_policy_id"
            ],
            "egress_firewall_policy_id": firewall_group_db["egress_firewall_policy_id"],
        ...
        if hasattr(firewall_group_db.standard_attr, "id"):
            ...
            resource_extend.apply_funcs("firewall_groups", res, firewall_group_db)
        return db_utils.resource_fields(res, fields)

    def _get_policy_ordered_rules(self, context, policy_id):
        query = (
            context.session.query(FirewallRuleV2)
    ...
    def make_firewall_group_dict_with_rules(self, context, firewall_group_id):
        ...
        if ingress_policy_id:
            firewall_group["ingress_rule_list"] = self._get_policy_ordered_rules(
        ...
        if egress_policy_id:
            firewall_group["egress_rule_list"] = self._get_policy_ordered_rules(
        ...
        if not fwr_db["shared"]:
            if fwr_db["tenant_id"] != fwp_db["tenant_id"]:
                raise f_exc.FirewallRuleConflict(
                    firewall_rule_id=fwr_db["id"], project_id=fwr_db["tenant_id"]
                )

    def _process_rule_for_policy(
        self, context, firewall_policy_id, firewall_rule_id, position, association_db
    ):
        with db_api.CONTEXT_READER.using(context):
            fwp_query = context.session.query(FirewallPolicy).with_for_update()
            ...
            if position:
                ...
                fwp_db.rule_associations.insert(
                    position - 1,
                    FirewallPolicyRuleAssociation(firewall_rule_id=firewall_rule_id),
                )
            else:
    ...
    def _get_policy_rule_association_query(
        self, context, firewall_policy_id, firewall_rule_id
    ):
        fwpra_query = context.session.query(FirewallPolicyRuleAssociation)
        return fwpra_query.filter_by(
            firewall_policy_id=firewall_policy_id, firewall_rule_id=firewall_rule_id
        )
    ...
    def _ensure_rule_not_already_associated(
        ...
        try:
            self._get_policy_rule_association_query(
                context, firewall_policy_id, firewall_rule_id
            ).one()
            raise f_exc.FirewallRuleAlreadyAssociated(
                firewall_rule_id=firewall_rule_id, firewall_policy_id=firewall_policy_id
            )
        except exc.NoResultFound:
            return

    def _get_policy_rule_association(
        self, context, firewall_policy_id, firewall_rule_id
    ):
    ...
    def _create_default_firewall_rules(self, context, tenant_id):
        ...
        in_fwr_v4 = {
            ...
            "source_ip_address": cfg.CONF.default_fwg_rules.ingress_source_ipv4_address,
            "destination_port": cfg.CONF.default_fwg_rules.ingress_destination_port,
            "destination_ip_address": cfg.CONF.default_fwg_rules.ingress_destination_ipv4_address,
        }
        ...
        eg_fwr_v4["source_port"] = cfg.CONF.default_fwg_rules.egress_source_port
        ...
        eg_fwr_v6["source_port"] = cfg.CONF.default_fwg_rules.egress_source_port
    ...
    def insert_rule(self, context, id, rule_info):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            if ref_firewall_rule_id:
                ...
                position = 1
            return self._process_rule_for_policy(
                context, id, firewall_rule_id, position, None
            )
    ...
    def remove_rule(self, context, id, rule_info):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            fwpra_db = self._get_policy_rule_association(context, id, firewall_rule_id)
            return self._process_rule_for_policy(
                context, id, firewall_rule_id, None, fwpra_db
            )
    ...
    def _get_rules_in_policy(self, context, fwpid):
        ...
        with db_api.CONTEXT_READER.using(context):
            fw_pol_rule_qry = context.session.query(
                FirewallPolicyRuleAssociation
            ).filter_by(firewall_policy_id=fwpid)
    ...
    def get_policies_with_rule(self, context, fwrid):
        ...
        with db_api.CONTEXT_READER.using(context):
            fw_pol_rule_qry = context.session.query(
                FirewallPolicyRuleAssociation
            ).filter_by(firewall_rule_id=fwrid)
    ...
    def _check_rules_for_policy_is_valid(
        self, context, fwp, fwp_db, rule_id_list, filters
    ):
            ...
            if "shared" in fwp:
                if fwp["shared"] and not rules_dict[fwrule_id]["shared"]:
                    raise f_exc.FirewallRuleSharingConflict(
                        firewall_rule_id=fwrule_id, firewall_policy_id=fwp_db["id"]
                    )
                ...
                if not rules_dict[fwrule_id]["shared"]:
                    if rules_dict[fwrule_id]["tenant_id"] != fwp_db["tenant_id"]:
                        raise f_exc.FirewallRuleConflict(
    ...
    def _check_if_rules_shared_for_policy_shared(self, context, fwp_db, fwp):
        if fwp["shared"]:
            ...
            for entry in rules_in_db:
                fwr_db = self._get_firewall_rule(context, entry.firewall_rule_id)
                if not fwr_db["shared"]:
                    raise f_exc.FirewallPolicySharingConflict(
                        firewall_rule_id=fwr_db["id"], firewall_policy_id=fwp_db["id"]
                    )

    def get_fwgs_with_policy(self, context, fwp_id):
        with db_api.CONTEXT_READER.using(context):
            fwg_ing_pol_qry = context.session.query(FirewallGroup).filter_by(
                ingress_firewall_policy_id=fwp_id
            )
            ing_fwg_ids = [entry.id for entry in fwg_ing_pol_qry]
            fwg_eg_pol_qry = context.session.query(FirewallGroup).filter_by(
    ...
    def _check_fwgs_associated_with_policy_in_same_project(
        ...
        with db_api.CONTEXT_READER.using(context):
            fwg_with_fwp_id_db = context.session.query(FirewallGroup).filter(
                or_(
                    FirewallGroup.ingress_firewall_policy_id == fwp_id,
    ...
    def _delete_all_rules_from_policy(self, context, fwp_db):
        ...
        """
        for rule_id in [
            rule_assoc.firewall_rule_id for rule_assoc in fwp_db["rule_associations"]
        ]:
            fwpra_db = self._get_policy_rule_association(context, fwp_db["id"], rule_id)
    ...
    def _set_rules_for_policy(self, context, firewall_policy_db, fwp):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            self._check_rules_for_policy_is_valid(
                context, fwp, fwp_db, rule_id_list, filters
            )
            ...
            rules_in_fpol_rul_db = model_query.get_collection_query(
                context, FirewallPolicyRuleAssociation, filters=filters
            )
            rules_dict = dict(
                (fpol_rul_db["firewall_rule_id"], fpol_rul_db)
    ...
    def _create_default_firewall_policy(
        self, context, tenant_id, policy_type, **kwargs
    ):
        fwrs = kwargs.get("firewall_rules", [])
        description = kwargs.get("description", "")
        name = (
            const.DEFAULT_FWP_INGRESS
            if policy_type == "ingress"
            else const.DEFAULT_FWP_EGRESS
        )
        firewall_policy = {
            "name": name,
    ...
    def update_firewall_policy(self, context, id, firewall_policy):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            self._ensure_not_default_resource(
                fwp_db, "firewall_policy", action="update"
            )
            ...
            if "shared" in fwp and "firewall_rules" not in fwp:
                self._check_if_rules_shared_for_policy_shared(context, fwp_db, fwp)
    ...
    def get_ports_in_firewall_group(self, context, firewall_group_id):
        ...
        with db_api.CONTEXT_READER.using(context):
            fw_group_port_qry = context.session.query(FirewallGroupPortAssociation)
    ...
    def _delete_ports_in_firewall_group(self, context, firewall_group_id):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            fw_group_port_qry = context.session.query(FirewallGroupPortAssociation)
            fw_group_port_qry.filter_by(firewall_group_id=firewall_group_id).delete()
        return

    def _get_default_fwg_id(self, context, tenant_id):
        """Returns an id of default firewall group for given tenant or None"""
        default_fwg = (
            model_query.query_with_hooks(context.elevated(), FirewallGroup)
            .filter_by(project_id=tenant_id, name=const.DEFAULT_FWG)
            .first()
        )
        if default_fwg:
    ...
    def get_fwg_attached_to_port(self, context, port_id):
        ...
        fwg_port = (
            model_query.query_with_hooks(context, FirewallGroupPortAssociation)
    ...
    def get_fwg_ports_in_tenant(self, context, tenant_id):
        ...
        try:
            ...
            with db_api.CONTEXT_READER.using(context):
                port_qry = (
                    context.session.query(FirewallGroupPortAssociation.port_id)
                    .join(FirewallGroup, FirewallGroup.id == fwg_id)
                    .filter(FirewallGroup.tenant_id == tenant_id)
                    .all()
                )
    ...
    def _ensure_default_firewall_group(self, context, tenant_id):
        ...
        try:
            ...
            with db_api.CONTEXT_WRITER.using(ctx):
                ...
                fwr_ids = self._create_default_firewall_rules(ctx, tenant_id)
                ingress_fwp = {
                    "description": "Ingress firewall policy",
                    "firewall_rules": [fwr_ids["in_ipv4"], fwr_ids["in_ipv6"]],
                }
                egress_fwp = {
                    "description": "Egress firewall policy",
                    "firewall_rules": [fwr_ids["eg_ipv4"], fwr_ids["eg_ipv6"]],
                }
                ...
                fwg_db = self._create_firewall_group(ctx, fwg, default_fwg=True)
                ctx.session.add(
                    DefaultFirewallGroup(
                        firewall_group_id=fwg_db["id"], project_id=tenant_id
                    )
                )
                return fwg_db["id"]
    ...
    def _create_firewall_group(self, context, firewall_group, default_fwg=False):
    ...
    def update_firewall_group(self, context, id, firewall_group):
        ...
        with db_api.CONTEXT_WRITER.using(context):
                ...
                for attr in attrs:
                    if attr in fwg:
                        raise FirewallDefaultObjectUpdateRestricted(
                            resource_type="Firewall Group", resource_id=fwg_db["id"]
                        )
            if "ports" in fwg:
            ...
            if fwg:
                fwg_db.update(db_utils.filter_non_model_columns(fwg, FirewallGroup))
    ...
    def update_firewall_group_status(self, context, id, status, not_in=None):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            return (
                context.session.query(FirewallGroup)
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            if _is_default(fwg_db):
                if context.is_admin:
                    ...
                    context.session.query(FirewallGroup).filter_by(id=id).delete()
                    fwp = [
                        fwg_db["ingress_firewall_policy_id"],
    ...
    def _ensure_not_default_resource(self, resource_dict, r_type, action=None):
        ...
        if r_type == "firewall_group":
            if resource.get("name", "") == const.DEFAULT_FWG:
                if action == "update":
                    raise FirewallDefaultObjectUpdateRestricted(
                        resource_type="Firewall Group", resource_id=resource["id"]
                    )
